...
def get_options_pricing(pricing_model):
    ...
    def calculate_option_price(event):
        ...
        if event:
            ...
            if pricing_model == OPTION_PRICING_MODEL.BINOMIAL_TREE.value:
                options_pricing = BinomialTreeModel(spot_price, strike_price.value, risk_free_rate.value, sigma.value, (exercise_date.value - dt.date.today()).days, number_of_time_steps.value)
            elif pricing_model == OPTION_PRICING_MODEL.BLACK_SCHOLES.value:
                options_pricing = BlackScholesModel(spot_price, strike_price.value, (exercise_date.value - dt.date.today()).days, risk_free_rate.value, sigma.value)
            title_tabularor = pn.pane.Markdown(f"# Stock Price Data for {ticker.value}")
            title_plot = pn.pane.Markdown(f"# Stock Price Chart for {ticker.value}")
            tabulator = pn.widgets.Tabulator(historical_data, pagination="remote", page_size=10, disabled = True, width=800)
            tabulator = pn.Column(title_tabularor, tabulator)
            plot = historical_data.hvplot.line(x='Date', y='Close', title=f'{ticker.value} Stock Price', width=800, height=400)
            plot = pn.Column(title_plot, plot)
            call_option_price = options_pricing.calculate_option_price("Call")
            put_option_price = options_pricing.calculate_option_price("Put")
            markdown_content = f"""
